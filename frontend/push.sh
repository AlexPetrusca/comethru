...
# Script to build and push the frontend image to Docker Hub
...
if [ "$ENVIRONMENT" == "prod" ]; then
  ...
  docker tag alexpetrusca/comethru-frontend:$IMAGE_TAG alexpetrusca/comethru-frontend:latest
fi
...
if [ "$ENVIRONMENT" == "prod" ]; then
  docker push alexpetrusca/comethru-frontend:latest
fi

echo "Image(s) pushed successfully!"
echo "Image: alexpetrusca/comethru-frontend:$IMAGE_TAG"
if [ "$ENVIRONMENT" == "prod" ]; then
  echo "Also tagged as: alexpetrusca/comethru-frontend:latest"
fi